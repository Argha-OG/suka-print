            ...
            {items.map((item, idx) => {
                ...
                return (
                    <Link href={`/products/${item._id}`} key={idx} className="flex gap-4 group cursor-pointer">
                        ...
                        <div>
                            <h4 className="font-medium text-gray-900 mb-1 line-clamp-2 group-hover:text-primary-blue transition-colors">
                                {item.title || "Premium Product"}
                            </h4>
                            <div className="flex text-yellow-400 text-xs mb-1">
                                {[...Array(5)].map((_, i) => <Star key={i} size={12} fill="currentColor" />)}
                            </div>
                            <div className="font-bold text-primary-magenta text-sm">RM{price.toFixed(2)}</div>
                        </div>
                    </Link>
                );
            })}
...
const HomeProductLists = ({ featuredData }) => {
    if (!featuredData || featuredData.length === 0) {
        return null; // Don't show anything if no DB data
    }

    // Get distinct sets of products for each list
    const bestSellers = featuredData.slice(0, 3);
    const topSales = featuredData.length >= 6 ? featuredData.slice(3, 6) : featuredData.slice(0, 3);
    const topRated = featuredData.length >= 9 ? featuredData.slice(6, 9) : featuredData.slice(0, 3);
        ...
        <section className="mb-16">
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-8">
                    <ProductList title="Best Seller" items={bestSellers} />
                    <ProductList title="Top Sale" items={topSales} />
                    <ProductList title="Top Rated" items={topRated} />
                </div>
                ...
                <div className="bg-orange-100 rounded-xl p-8 flex flex-col items-center text-center justify-center min-h-[400px] relative overflow-hidden group">
                    ...
                    <p className="text-gray-600 mb-6 text-sm">Start from <span className="text-xl font-bold text-gray-900">RM59.00</span></p>
