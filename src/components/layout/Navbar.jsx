    ...
    useEffect(() => {
        const fetchSettings = async () => {
            try {
                ...
                console.error('Failed to fetch navbar settings');
            }
        };
        fetchSettings();
    }, []);
        ...
        <header className="fixed top-0 left-0 w-full z-50 transition-all duration-300">
            ...
            <div className="bg-primary-blue/90 text-white text-xs py-1 px-4 md:px-8 flex justify-between items-center backdrop-blur-sm">
                <div className="flex items-center gap-4">
                    <span className="flex items-center gap-1"><MapPin size={12} /> {settings?.address || 'Kuala Lumpur, Malaysia'}</span>
                    <span className="hidden md:inline">Free Shipping on Orders Over RM100</span>
                </div>
                <div className="flex items-center gap-3">
                    <a href={settings?.socialLinks?.facebook || "#"} className="hover:text-accent"><Facebook size={12} /></a>
                    <a href={settings?.socialLinks?.instagram || "#"} className="hover:text-accent"><Instagram size={12} /></a>
                    <a href={settings?.socialLinks?.twitter || "#"} className="hover:text-accent"><Twitter size={12} /></a>
                </div>
            </div>
            ...
            <div className="bg-white backdrop-blur-md border-b border-gray-100 shadow-sm">
                <div className="container mx-auto px-4 md:px-8 py-0.5 flex items-center justify-between gap-4">
                    ...
                    <Link href="/" className="flex items-center">
                        <img src={settings?.logo || logoDefault} alt={`${brandName} - Premium Printing Services Malaysia`} className="h-[68px] w-auto object-contain" />
                    </Link>
                                ...
                                {searchResults.length > 0 ? (
                                    <ul>
                                        {searchResults.map(product => (
                                            ...
                                            >
                                                <img src={product.image} alt={`${product.title} printing`} className="w-10 h-10 object-cover rounded-md bg-gray-100" />
                                                <div>
                    ...
                    <div className="flex items-center gap-2 md:gap-4">
                        <div className="hidden md:flex flex-col items-end text-xs text-gray-600 mr-2">
                            ...
                            <span>{settings?.contactPhone || '+60 11-1414 1509'}</span>
                        </div>
                ...
                <div className="container mx-auto px-4 md:px-8">
                    <div className="flex items-center gap-8">
                        ...
                        <ul className="flex items-center gap-8 py-3 text-sm font-medium text-gray-700 flex-1">
                            <li><Link href="/" className="hover:text-primary-blue cursor-pointer font-semibold">Home</Link></li>
                            <li><Link href="/products" className="hover:text-primary-blue cursor-pointer">Products</Link></li>
                            ...
                            <li><Link href="/about" className="hover:text-primary-blue cursor-pointer">About Us</Link></li>
                            <li><Link href="/contact" className="hover:text-primary-blue cursor-pointer">Contact</Link></li>
                        </ul>
