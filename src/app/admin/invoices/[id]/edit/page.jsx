...
const EditInvoice = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setTaxRate(order.taxAmount || 0); // Assuming stored as percentage
  ...
  }, [advanceAmount, items, taxRate, loading]);
  ...
  const handleDelete = async () => {
    if (!window.confirm(`Are you sure you want to delete invoice ${orderNumber}?`)) return;
    
    setSaving(true);
    try {
      console.log(`[DELETE] Attempting to delete invoice ${id} from edit page`);
      await api.delete(`/orders/${id}`);
      alert("Invoice deleted successfully!");
      router.push("/admin/invoices");
    } catch (error) {
      console.error("[DELETE] Error on edit page:", error);
  ...
  const handleSave = async (downloadPDF = false) => {
    ...
    try {
      ...
      const orderData = {
        ...
        taxAmount: taxRate || 0, // Storing percentage
        status: finalStatus,
      };
      ...
      if (downloadPDF) {
        try {
          await generatePDF(orderNumber);
        } catch (pdfError) {
          console.error("PDF generation failed", pdfError);
          alert("Invoice updated, but PDF generation failed. You can try downloading it later.");
        }
      }
  ...
  const generatePDF = async (num) => {
    try {
      ...
      const fileName = `${num || "invoice"}.pdf`;
      pdf.save(fileName);
    } catch (error) {
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-black text-gray-900">
            Edit Invoice #{orderNumber}
          </h1>
          <p className="text-gray-500">
            Modify the invoice details and items below.
          </p>
        </div>
          ...
          <Card className="p-6 border-none shadow-lg bg-white space-y-4">
            ...
            <div className="space-y-3">
              ...
              <Button
                ...
                disabled={saving}
                variant="outline"
                className="w-full flex items-center justify-center gap-2 border-red-200 text-red-600 hover:bg-red-50 h-12 rounded-xl mt-4"
              >
